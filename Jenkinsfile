                ...
                withCredentials([string(credentialsId: 'wsl-host-ip', variable: 'WSL_HOST')]) {
                    sshagent(['wsl-ssh-creds']) {
                        bat """
                            ...
                            scp -o StrictHostKeyChecking=no ./scripts/deploy-to-minikube.sh ${WSL_USERNAME}@${WSL_HOST}:~/studyshare/scripts/deploy-to-minikube.sh
                            ssh -o StrictHostKeyChecking=no ${WSL_USERNAME}@${WSL_HOST} "sed -i 's/\\r//g' ~/studyshare/scripts/deploy-to-minikube.sh"
                            ssh -o StrictHostKeyChecking=no ${WSL_USERNAME}@${WSL_HOST} "bash ~/studyshare/scripts/deploy-to-minikube.sh"
                        """
                    }
                }
            }
