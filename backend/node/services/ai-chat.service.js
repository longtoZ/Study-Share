    ...
    async generateResponse(userId, materialId, message, model) {
        ...
        if (!chat) {
            chat = gemini.chats.create({
                model,
                history: [
                    { role: 'user', parts: [{text: 'You are a helpful assistant. Always provide concise, complete responses within 10 sentences. For any non-english queries, respond in the same language as the prompt unless otherwise specified by the user. Summarize complex topics clearly and avoid abrupt endings.' }] }
                ],
                config: { temperature: 0.7, maxOutputTokens: 512, responseMimeType: 'text/plain' }
            });
